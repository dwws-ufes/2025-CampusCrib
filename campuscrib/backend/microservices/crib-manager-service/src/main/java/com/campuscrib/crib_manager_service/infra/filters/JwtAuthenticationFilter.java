package com.campuscrib.crib_manager_service.infra.filters;
    ...
    protected void doFilterInternal(
        ...
        if (StringUtils.hasText(token)) {
            try {
                ...
                if (authTokenData.getRole() != UserRole.LANDLORD) {
                    response.setStatus(HttpServletResponse.SC_FORBIDDEN);
                    response.setContentType("application/json");
                    response.getWriter().write("{\"message\": \"Access denied: only Landlord is allowed\"}");
                    return;
                }
